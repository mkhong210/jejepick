...
export async function GET(req){
    ...
    if(status === 'idcheck'){//idcheck =회원가입 시 아이디 중복확인 체크
        const id = req.nextUrl.searchParams.get('id'); // 아이디 파라미터를 가져옴
        const existingUsers = await queryExecute ('SELECT * from jejumembership where id=?',[id])

        return Response.json(existingUsers);
    }else{// 그게 아니라면 로그인 할때 아이디와 패스워드 값을 가져와야함

        /* await queryExecute ('SELECT * from jejumembership where id=? & pw=?',[id,password]) */

        const id = req.nextUrl.searchParams.get('id'); // 아이디 파라미터를 가져옴
        const pw = req.nextUrl.searchParams.get('pw'); // 패스워드 파라미터를 가져옴
        const user = await queryExecute('SELECT * FROM jejumembership WHERE id = ? AND pw = ?', [id, pw]);
        console.log(user);
        if (user.length > 0) { //0보다 크면 하나의 정보가 담기므로 로그인 성공을 뜻함
            // 로그인 성공
