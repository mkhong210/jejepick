...
//======================전 페이지에서 코스 가져오기==================
    ...
    useEffect(()=> {
        if(apiData && idString){
            ...
            const filterdata = apiData.filter((item) => idArray.includes(item.contentsid)); 
...
useEffect(()=>{
    ...
    */
    ...
    kakaoMapScript.src = `https://dapi.kakao.com/v2/maps/sdk.js?appkey=700d399006256f95732f06b19c046ba5&autoload=false`
    ...
    setTimeout(()=>{
        ...
        if(sections){
            ...
            allMarker.forEach( (obj,k)=>{
                let num = 0;
                obj.name == '출발지'? num=0 : (obj.name == '목적지' ? num=2 : num=1);
                if(k != 1 && k != 2)
